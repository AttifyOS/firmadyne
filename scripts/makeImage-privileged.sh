#!/bin/bash
# For privileged LXD container (Privileged Docker/Podman has no issues running original makeImage.sh script but can also run this one)
set -e
set -u
...
# Device node can't be created on unprivileged containers with mknod
# mknod can be used on privileged containers though
